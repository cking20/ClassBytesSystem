...
//Perfomr dB query of all user student contacts prior to user action to provide pre-filled form fields
elseif(empty($_POST['submit']))
{
	...
	$db = mysql_select_db('CollegeBytes', $connection) or die ("Couldn't select database.");
		...
		$data = "SELECT `Student_Contact_ID`, `Student_Contact_Name`, `Student_Phone`, `Student_Email` FROM Student_Contact WHERE Student_Contact_ID = '$editCont' AND User_ID = '$username'"; 
...
{
		//Upon user action in posting a student contact edit, validates all data fields
        if(!empty($_POST['submit']))
		{
					...
					$sName = trim($_POST['name']);
						if(empty($sName))
						{
								$tName_error = "Please enter a student contact name";
								$error = true;
						}
			...
			//If all data fields are valid, moves new data into an update query to update the student contact information	
			if(false === $error){		
				...
				$query1 = "UPDATE Student_Contact SET Student_Contact_Name = '$sName', Student_Phone = '$phNum', Student_Email = '$sEmail' WHERE Student_Contact_ID = '$oldID' AND User_ID = '$username'";
...
<form name = "feedback" id = "addTeacher" action = "?" method ="post">
	...
	<fieldset>
		...
		<label><input type = "hidden" name = "oldID" value = <?php echo $change[Student_Contact_ID] ?>></label>
			
		<label for= "Name">Name: </label><br> <input name = "name" type = "text" value="<?php echo $change[Student_Contact_Name]?>"></br>
		<span class="error"><?php echo $tName_error ?></br></span>

		<label for="Phone Number">Phone Number: <br><input type="tel" name="phNum" value="<?php echo $change[Student_Phone]?>"></label></br></br>
		<span class="error"><?php echo $phNum_error ?></br></span>

		<label for= "Email Address">Email Address:<br> <input type= "email" name="emailAdd" value="<?php echo $change[Student_Email]?>"></label> </br></br>
