    ...
    try
    {
        if(!empty($_POST['submit']))
		{
					...
					$sName = trim($_POST['name']);
						if(empty($sName))
						{
								$sName_error = "Please enter a student contact name";
								$error = true;
						}
			...
			if(false === $error){		
				...
				$query = "INSERT INTO Student_Contact (`Student_Contact_Name`, `Class_Name`, `Student_Phone`, `Student_Email`, `User_ID`) 
							VALUES ('$sName', '$cName', '$phNum', '$sEmail', '$username')";
					...
					if($result = $link->query($query))
						{
							...
							$confMsg = "You have successfully added $sName, in your $cName class, to your account";
						}
						else
						{
...
<form name = "feedback" id = "addStuCont" action = "?" method ="post">
<h3>Add a Student Contact</h3>
<p>You may only add Student Contacts for classes you have entered into your schedule.  
To edit or delete an Student Contact, please go to the relavant class page and select "Edit" or "Delete".</p>
	<fieldset>
		<label for= "Name">Name: </label><br> <input name = "name" type = "text"></br>
		<span class="error"><?php echo $sName_error ?></br></span>
